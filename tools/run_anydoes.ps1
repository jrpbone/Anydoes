...
function Test-SymbolicLinkSupport {
    $testDirectory = Join-Path ([System.IO.Path]::GetTempPath()) "anydoes-symlink-test-$PID"
...
void main(List<String> arguments) {
  ...
  target.writeAsStringSync('Anydoes symlink capability test');
...
'@ -ForegroundColor Yellow
    ...
    if (-not [string]::IsNullOrWhiteSpace($openSettings) -and $openSettings -notmatch '^(y|yes)$') {
        throw 'Developer Mode is required before Anydoes can run with Flutter plugins.'
    }
...
try {
    ...
    if (-not (Get-Command flutter -ErrorAction SilentlyContinue)) {
        throw 'Flutter was not found on PATH. Install Flutter and restart PowerShell before running Anydoes.'
    }

    $pubspecPath = Join-Path -Path $ProjectDirectory -ChildPath 'pubspec.yaml'
    if (-not (Test-Path -LiteralPath $pubspecPath)) {
        throw "Anydoes pubspec.yaml was not found in '$ProjectDirectory'."
    }
    ...
    if ($FlutterArguments) { $runArguments += $FlutterArguments }

    Write-Host "Starting Anydoes on $($selectedDevice.name)..." -ForegroundColor Green
