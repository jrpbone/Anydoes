...
function Test-SymbolicLinkSupport {
    ...
    $testDirectory = Join-Path ([IO.Path]::GetTempPath()) "anydoes-symlink-test-$PID"
...
void main(List<String> args) {
  File(args[0]).writeAsStringSync('Anydoes symlink test');
...
try {
    ...
    if ($Mode -eq 'release' -and -not (Test-Path -LiteralPath $keyPropertiesPath)) {
        throw 'Release signing is not configured. Run .\setup_android_signing.ps1, then retry.'
    }
    ...
    Write-Host "`nAnydoes Android build" -ForegroundColor Green
    Write-Host '----------------------------------------'
    ...
    if (-not $NonInteractive -and -not (Read-YesNo -Prompt 'Proceed?' -DefaultValue $true)) {
    ...
    Invoke-FlutterStep -Title 'Building Anydoes for Android...' -Arguments $arguments
    ...
    if ([string]::IsNullOrWhiteSpace($safeName)) { $safeName = 'Anydoes' }
